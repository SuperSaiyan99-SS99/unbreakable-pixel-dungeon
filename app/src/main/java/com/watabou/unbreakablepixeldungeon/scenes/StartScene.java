...
import com.watabou.unbreakablepixeldungeon.effects.BannerSprites.Type;
...
import com.watabou.unbreakablepixeldungeon.windows.WndChallenges;
import com.watabou.unbreakablepixeldungeon.windows.WndClass;
import com.watabou.unbreakablepixeldungeon.windows.WndMessage;
import com.watabou.unbreakablepixeldungeon.windows.WndOptions;
...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		curClass = null;
		updateClass( HeroClass.values()[UnbreakablePixelDungeon.lastClass()] );
		...
		Badges.loadingListener = new Callback() {
			...
			public void call() {
				if (Game.scene() == StartScene.this) {
					UnbreakablePixelDungeon.switchNoFade( StartScene.class );
				}
			}
		};
	}
	...
	private void startNewGame() {
		...
		if (UnbreakablePixelDungeon.intro()) {
			UnbreakablePixelDungeon.intro( false );
	...
	private class ChallengeButton extends Button {
		...
		protected void createChildren() {
			...
			image = Icons.get( UnbreakablePixelDungeon.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF );
			add( image );
		}
		...
		protected void onClick() {
			if (Badges.isUnlocked( Badges.Badge.VICTORY )) {
				StartScene.this.add( new WndChallenges( UnbreakablePixelDungeon.challenges(), true ) {
					public void onBackPressed() {
						super.onBackPressed();
						image.copy( Icons.get( UnbreakablePixelDungeon.challenges() > 0 ?
